...
远程访问使用 avibe 自带的 `vibe remote` 能力，默认后端是 `https://avibe.zengsipei.cn`。首次使用需要 pairing key：

```bash
docker compose exec avibe vibe remote pair --backend-url https://avibe.zengsipei.cn --device-name avibe-container <pairing_key>
docker compose exec avibe vibe remote status
```
...
| `AVIBE_REMOTE_START` | `true` | 启动时是否尝试启动 avibe remote tunnel |
| `AVIBE_REMOTE_BACKEND_URL` | `https://avibe.zengsipei.cn` | `vibe remote pair --backend-url` 使用的后端地址 |
| `AVIBE_REMOTE_DEVICE_NAME` | `avibe-container` | remote pairing 显示的设备名 |
| `AVIBE_REMOTE_PAIRING_KEY` | 空 | 可选的一次性 pairing key；通常手动执行 `vibe remote pair` 更安全 |
...
- `cloudflared`: 由 avibe remote 使用，用于建立官方 remote tunnel。
